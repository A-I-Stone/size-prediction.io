...
function calculateShirtSizeForBoys(heightFeet, heightInches, weight, age, waistSize) {
...
function calculateFitForBoys(heightFeet, heightInches, weight, age) {
    ...
    const percentilesHeight = heightDataBoys[age + " years"];
    const percentilesWeight = weightDataBoys[age + " years"];
    ...
    if (averagePercentile <= 25) return "extra slim";
    if (averagePercentile <= 50) return "slim";
    if (averagePercentile <= 75) return "classic";
    if (averagePercentile <= 85) return "traditional";
    return "husky";
}
...
console.log(`Recommended fit: ${fit}`);
